...
if (!empty($app_title) && !empty($app_logo) && !empty($last_values_limit) && !empty($present_data_limit) && !empty($sys_border_high) && !empty($sys_border_low) && !empty($dia_border_high) && !empty($dia_border_low) && !empty($pulse_border_high) && !empty($pulse_border_low) && !empty($token)) {
	...
	if (check_access($token, array(ADMIN, OPERATOR), $db_connection)) {
		...
		$query = "UPDATE settings SET key_value = :app_logo, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :last_values_limit, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :present_data_limit, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :sys_border_high, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :sys_border_low, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :dia_border_high, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :dia_border_low, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :pulse_border_high, modified = NOW()" .
		...
		$query = "UPDATE settings SET key_value = :pulse_border_low, modified = NOW()" .
